...
#| echo: false
#| output: false
using Pkg; Pkg.activate(abspath(@__DIR__));
```
...
automatic differentiation and probabilsitic programming toolchains, we will
...
- Numerical errors in backpropogation
...
to konw that every source of uncertainty is contained. In some cases we have
...
dual numbers, to then propogate through whole programs. This idea comes from the
arthmetic on normally distributed random variables. If we interpret $x \sim N(a,b)$,
...
recompile to do the arithemetic of uncertainty propogation. For example, the
following solves the pendulum of arbitrary amplitude with respect to uncertain
parameters and initial condtions:
...
polynomial expsnsions. This is the *polynomial chaos expansion* approach which
...
numerical uncertainty? This is done via *interval arithemetic*.
...
From here, non-monotone functions can propogate intervals like:
...
of primatives which is known to then propogate forward the intervals.
...
would expect given the actual uncertainties seen in practice. One phonomena
which causes this can be seen by looking at that pendulum:

```{julia}
g = 9.77..9.81; # Gravitational constants
...
```{julia}
g = 9.8..9.8; # Gravitational constants
...
with reverse-mode AD, can we instead look for higher order uncertainty primatives
...
That's all for adapting to a tolerance, but can we use this to propogate
...
approximation to the possible values that could have occured given the noise
...
distribution of the output of the program, and then compute quantites from that.
...
uncertainty density $f$, we can reprsent calculating the expected value of some
